	...
	\subsection{Mutually-Exciting Poisson Processes}
		...
		\begin{algorithm}
				\begin{algorithmic}
					...
					\REPEAT
						\FOR{$s_m$ in $S$}
							\STATE{$S_m \gets$ Generate sample $\{s_k\}_{k=1}^{M_m}$ according to $\lambda(t) = \phi(t - s_m)$ on $[s_m, T]$}
						\ENDFOR
		...
		For the purposes of simulating a mutually-exciting Poisson process, it is useful to associate each event with a Poisson process having an intensity
		%
		\begin{equation}
		...
		\begin{algorithm}
				\begin{algorithmic}
					...
					\REPEAT
						\FOR{$s_m, c_m$ in $S$}
							\STATE{$(S_m, C_m) \gets$ Generate sample $\{s_k, c_k\}_{k=1}^{M_m}$ according to $\lambda_{m,n}(t)$ on $[s_m, T]$}
						\ENDFOR
